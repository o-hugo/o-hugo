```
bacharelando · estatística · UFAM
manaus · -3.10°, -60.02°
```

estatístico em formação. transformo ruído em sinal com pipelines reprodutíveis,
controle de processo e séries temporais.

![Teorema de Bayes](https://latex.codecogs.com/svg.image?{\color{Gray}P(A\mid%20B)=\dfrac{P(B\mid%20A)\cdot%20P(A)}{P(B)}})

---

## sobre

```
linguagens   Python · R · SQL · JavaScript
bibliotecas  DuckDB · Pandas · Statsmodels · Scikit-learn · Chart.js
métodos      CEP · STL · séries temporais · inferência bayesiana
```

acredito que boas análises são tão importantes quanto boas visualizações.

---

## estatísticas

![stats](./profile/stats.svg)
![top languages](./profile/top-langs.svg)

---

[portfolio](https://o-hugo.github.io/portfolio/) · [linkedin](https://linkedin.com/in/hugo-matheus-637bb0350) · hugo-matheus.rocha@ufam.edu.br

```
p << 0.001 · built with care
```
